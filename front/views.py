...
def quiz_detail(request, code):
    ...
    if current_time <= quiz.end_time:
        if quiz.is_active:
            ...
            return HttpResponse("Qabul vaqti to`xtatilgan")
    else:
        return HttpResponse('Qabul qilish vaqti tugagan')

def quiz_link_copy(request, code):
    data_str = str(code)
...
def create_answers(request, code):
    ...
    answers = models.Answer.objects.filter(taker=quiz_taker)
    context = {
        'taker':quiz_taker,
        'answers':answers
    }
    return render(request, 'front/result-detail.html', context)
